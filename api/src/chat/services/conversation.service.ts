...
export class ConversationService {
  ...
  getConversationsForUser(userId: number): Observable<Conversation[] | any> {
    return from(
      this.conversationRepository
        .createQueryBuilder('conversation')
        .leftJoin('conversation.users', 'user')
  ...
  getUsersInConversation(conversationId: number): Observable<Conversation[] | any> {
    return from(
      this.conversationRepository
        .createQueryBuilder('conversation')
  ...
  getMessages(conversationId: number): Observable<Message[] | any> {
    return from(
...
//             .createQueryBuilder('conversation')
//             .innerJoinAndSelect('conversation.users', 'user')
//             .where('user.id = :conversationId', { conversationId })
//             .getMany(),
//         )
//     }

//     getConversationsWithUsers(userId: number): Observable<Conversation[] | any> {
//         return from(this.getConversationsForUser(userId)).pipe(
//             take(1),
//             switchMap((conversations: Conversation[]) => conversations),
//             mergeMap((conversation: Conversation) => {
